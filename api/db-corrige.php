...
echo "<div class='text-center mb-6'>
        ...
        <p class='text-xs text-slate-400 mt-1'>Sincronizando novas tabelas e colunas com segurança</p>
      </div>";
...
function executarMigracao($sql, $descricao) {
    ...
    try {
        ...
        // Se a coluna ou tabela já existir, tratamos de forma limpa para não assustar com erros normais de SQL
...
// 2. MIGRAÇÕES AUTOMÁTICAS DE COLUNAS NOVAS
...
// 3. GARANTIA DE INTEGRIDADE DO ADMINISTRADOR MASTER
...
try {
    ...
    // Verifica se já existe o master no banco de dados
    ...
    if ($stmt->rowCount() === 0) {
        ...
        // Se já existe, apenas garante as permissões e garante que o login dele continue ativo e autorizado
...
echo "<div class='mt-6 pt-4 border-t border-slate-800 text-center'>
        <p class='text-sm text-slate-300 font-semibold mb-3'>Migração concluída com sucesso!</p>
